...
3. There is some heterogeneity among historic districts, but in general, DC's HDs are significantly wealthier and whiter than the rest of the District. That said, moderate-income renters in HDs are more likely to be rent-burdened than moderate-income renters outside of HDs.
...
7. Overall, houses in HDs lag in terms of solar adoption, especially in detached house neighborhoods.
...
```{r, fig.height=8, fig.width=12}
  ggplot(hd_shp %>% 
         mutate(sig_year_mean = (sig_year_begin + sig_year_end) / 2,
                name_short = substr(LABEL, 1, 5))) +
  geom_text(aes(label=name_short, x=desig_date, y=1780), angle=45, position=position_jitter(width = 0, height = 15)) +
  geom_segment(
    aes(
...
We took a cursory look at this in DC, using [sale price](https://www.kaggle.com/datasets/christophercorrea/dc-residential-properties?select=DC_Properties.csv) data and found that, controlling for a bunch of other factors (house characteristics, zoning, the ward, local income and poverty), HD designation raises an average sale prices somewhere around 7-10%. (It's possible that our regression was still missing important predictor variables, but adding many additional variables did not chage the estimated relationship between HD-designation and sale price much). This 7-10% estimate is roughly in line with estimates from other cities. See code for details.
...
## Block-level changes from 1940-2020
...
As we discussed above, there are many factors (tangled up with HD creation) that could explain divergent trends in versus outside of HDs. People who live in HDs and create them may do other things to try and limit housing density, for example. So while it's likely that HD creation does directly lead to a relative decrease in, say, population density (as other papers have shown), what we'll be doing here is a little more descriptive. We're mainly going to look and see, do HDs become relatively less dense over time? Do they seem to accrue relatively more white residents?
...
In the appendix, we also implement a more complex differences-in-differences regression approach.

The interactive map below shows which blocks are included with a buffer of 100m and 800m, as well as the HD boundaries themselves. We're omitting small HDs (less than 10 acres) from this analysis, because they lack enough block overlap. (Grant Circle, which is basically one circle in Petworth, is one example).
...
* HDs seem to become less dense over time relative to nearby blocks after HD designation. This is the strongest result. HD creation appears to be associated with a relative drop in population density between 4 and 9 residents per acre, or 9-21% of the typical HD population density.
* The percentage of white residents also seems to increase after designation, while the percentage of black residents seems to decline.
...
my_plt <-
  dplyr::bind_rows(
            ...
            res50$buffer_ts %>%
              ...
              mutate(dist = ifelse(post==1, 50, -50)),
            ...
            res100$buffer_ts %>%
              ...
              mutate(dist = ifelse(post==1, 100, -100)),
            ...
            res200$buffer_ts %>%
                  ...
                  mutate(dist = ifelse(post==1, 200, -200)),
            ...
            res400$buffer_ts %>%
                  ...
                  mutate(dist = ifelse(post==1, 400, -400)),
            ...
            res800$buffer_ts %>%
                  ...
                  mutate(dist = ifelse(post==1, 800, -800))
              ) %>%
  ungroup() %>%
  group_by(treat, post, dist) %>%
  summarise(
            dist = mean(dist),
            pop_density = weighted.mean(pop_density, w = hd_area_acres),
            pct_black = weighted.mean(pct_black, w = hd_area_acres),
            pct_white = weighted.mean(pct_white, w = hd_area_acres)
            )
...
```{r}
# uncomment to look at differences in pre-designation densities in and outside of HDs:
# res100$buffer_ts %>%
#   filter(post==0) %>%
...
ggplot(my_plt, 
  ...
  scale_fill_discrete(name = "In HD (1=yes)") +
  xlab("Building age") +
  facet_wrap(~`building type`) +
  ggdark::dark_theme_gray() +
