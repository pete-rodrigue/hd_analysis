...
```{r}
run_all <- function(tracts=T, mp, buff_dist, threshold) {
  if(tracts) {
...
```{r}
run_all(tracts=F, mp=.25, buff_dist=.008, threshold=.1)
```

```{r}
run_all(tracts=F, mp=.4, buff_dist=.005, threshold=.1)
```


```{r}
run_all(tracts=F, mp=.25, buff_dist=.005, threshold=.4)
```



```{r}
run_all(tracts=F, mp=.6, buff_dist=.01, threshold=.1)
```
