...
```{r}
get_neighbor_tracts <- function(hd_shp, t_shp, buffer_dist, geos_in_hd, remove_tract_thresh, year) {
  ...
  tracts_to_remove <- hd_tract$geo_id[hd_tract$pct_area > remove_tract_thresh]
  neighboring_tracts <- i[!(i$geo_id %in% tracts_to_remove),]
  
  # finally, remove
  neighboring_tracts <- t_shp[t_shp$geo_id %in% neighboring_tracts$geo_id,]
...
nearby_tracts60 <- 
  get_neighbor_tracts(hd_shp=hd_shp, t_shp=t60_shp, buffer_dist=buff_dist, geos_in_hd=hd_geos70,threshold, 1960)
nearby_tracts70 <- 
  get_neighbor_tracts(hd_shp=hd_shp, t_shp=t70_shp, buffer_dist=buff_dist, geos_in_hd=hd_geos70,threshold, 1970)
nearby_tracts80 <- 
  get_neighbor_tracts(hd_shp=hd_shp, t_shp=t80_shp, buffer_dist=buff_dist, geos_in_hd=hd_geos80,threshold, 1980)
nearby_tracts90 <- 
  get_neighbor_tracts(hd_shp=hd_shp, t_shp=t90_shp, buffer_dist=buff_dist, geos_in_hd=hd_geos90,threshold, 1990)
gc()
nearby_tracts00 <- 
  get_neighbor_tracts(hd_shp=hd_shp, t_shp=t00_shp, buffer_dist=buff_dist, geos_in_hd=hd_geos00,threshold, 2000)
nearby_tracts10 <- 
  get_neighbor_tracts(hd_shp=hd_shp, t_shp=t10_shp, buffer_dist=buff_dist, geos_in_hd=hd_geos10,threshold, 2010)
nearby_tracts20 <- 
  get_neighbor_tracts(hd_shp=hd_shp, t_shp=t20_shp, buffer_dist=buff_dist, geos_in_hd=hd_geos20,threshold, 2020)
gc()
...
Now we're going to repeat all of that, but using block groups instead of tracts. We will have to rely on sightly different versions of the Census data to do this, downloaded from NHGIS rather than Open Data DC. We're also only going back to 1990, since that's the data I could find relatively easily.
...
rm(b90_df, b00_df, b10_df, b20_df)

plot(sf::st_geometry(b20_shp["n_black"]))
```
...
plot_geos_in_hds(b90_shp, hd_geos90$geos_in_hd$geo_id)
```
