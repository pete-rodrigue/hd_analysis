...
The material in this GitHub page was used to write a series of Greater Greater Washington pieces. While the quantitative analysis on this page is polished, some of the text and framing is still relatively rough. For both the polished stats and accompanying text, please see the GGWash posts, which were professionally reviewed and edited.
...
2. HDs tend to overlap more with "residential flat" (RF) zones (think rowhouses), because rowhouse areas are more likely to be in older parts of the District. Conversely, outside of HDs, there's both suburban zone and apartment zone area.
...
VERBOSE = FALSE
# whether to render charts in dark mode
DARK_STYLE = FALSE
...
# convert to UTM Zone 18N projection
...
bar_color = ifelse(DARK_STYLE, "white", "black")
ggplot(hd_shp %>% 
  ...
  geom_segment(
    ...
    alpha=.6, linewidth=2, color=bar_color) +
  ...
  list(
    if (DARK_STYLE) ggdark::dark_theme_gray() else NULL
  )
```
...
The graphs below show how much of the land area currently covered in residential and mixed use (MU) zones in 2025 has been covered by HDs over time, as new HDs were created. Again, expansions of existing HDs are not depicted.
...
ggplot(bg_23_shp, 
       aes(x=median_year_str_built_, fill=forcats::fct_rev(as.factor(bg_23_shp$in_hd)))) +
...
to_plot_pop_dens <-
  bg_23_shp %>% 
       mutate(area_meters = as.vector(sf::st_area(.))) %>%
        mutate(pop_density_acre = tot_pop_ / area_meters * M2_PER_ACRE)
...
```{r}
ggplot(vac_shp) + geom_density(aes(x=p_own, fill=as.factor(in_hd)), alpha=.4) + 
...
```{r vacancy_rates}
ggplot(vac_shp) + geom_density(aes(x=p_vac, fill=as.factor(in_hd)), alpha=.4) +
...
to_plot <-
  to_plot %>% 
         tidyr::pivot_longer(cols = -`In HD (1=yes)`) %>%
         filter(stringr::str_detect(name, "\\$75k"))

ggplot(to_plot) +
  geom_bar(aes(x=name, y=value, fill=as.factor(`In HD (1=yes)`)), 
           stat='identity',
           position='dodge') +
  geom_text(
    aes(x=name, label = value, y = value + 0.07, group=as.factor(`In HD (1=yes)`)),
    vjust = -.05,
    position = position_dodge(width = .9)
  ) +
  ggtitle("Both higher- and lower-income HH more rent burdened in HDs") +
  ylab("%") +
  xlab("") +
  scale_fill_discrete(name = "In HD (1=yes)", direction=-1) +
  list(
    if (DARK_STYLE) ggdark::dark_theme_gray() else NULL
  ) 
```
...
temp <-
  bg_23_summary %>% group_by(LABEL) %>% mutate(pw = max(value[variable=="pct_white"])) %>% 
         ungroup() %>% 
         filter((variable %in% c("pct_white", "pct_black", "pct_hispa", "pct_other"))) %>%
  mutate(LABEL = ifelse(LABEL=="outside HD", "🚩 Outside HD", LABEL)) %>% 
  mutate(LABEL = ifelse(LABEL=="Entire District", "🚩 Entire District", LABEL))
...
ggplot(temp, 
  ...
  list(
    if (DARK_STYLE) ggdark::dark_theme_gray(base_size = 10) else NULL
  )
```
...
```{r, fig.height=12, eval=EVALUATE_ADDITIONAL_CHUNKS}
temp <-
  bg_23_summary %>% group_by(LABEL) %>% filter(variable=="median_hhinc") %>%
  mutate(LABEL = ifelse(LABEL=="outside HD", "🚩 Outside HD", LABEL)) %>% 
  mutate(LABEL = ifelse(LABEL=="Entire District", "🚩 Entire District", LABEL))
...
leaflet() %>% 
  ...
  addPolygons(data=hd_temp %>% filter(!(LABEL %in% areas_to_omit)) %>% st_transform(4326),
              ...
              opacity = 1,
              color = "white",
              label=~paste0(LABEL, ": ", round(pct_change_hu, 0), "% (", raw_change_hu, " units)"),
              highlightOptions = highlightOptions(weight = 3,
                                                  color = "white",
                                                  bringToFront = FALSE
        ) 
...
ggplot(policy_scenarios, aes(x=forcats::fct_rev(scenario), y=increase_in_hu_2000_2020)) +
  ...
  list(
    if (DARK_STYLE) ggdark::dark_theme_gray(base_size = 11) else NULL
  )
...
leaflet() %>% 
  ...
  addLayersControl(
    overlayGroups = c("Today's detached house zones 'R-1x'", "Approx. areas w/ discriminatory covenants", "% non-white in 1940","FHA grades from the 30s", "FHA map"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>% 
...
my_plt <-
  ...
  ungroup() %>%
  arrange(PRIMARYPROPERTYTYPE_EPACALC, `building vintage`, `In HD (1=yes)`) %>%
  group_by(`building vintage`, 
           PRIMARYPROPERTYTYPE_EPACALC) %>%
  mutate(`number of buildings 2` = sum(`number of buildings`)) %>%
  ungroup() %>%
  filter(`number of buildings 2` >= 100) %>%
  ...
  ungroup() %>%
  group_by(`building type`, `building vintage`) %>%
  mutate(row_ind = row_number()) %>%
  ungroup() %>%
  group_by(`building type`) %>%
  mutate(`number of buildings 2` = ifelse(row_ind==2, 0, `number of buildings 2`)) %>%
  mutate(`number of buildings 3` = ifelse(row_ind==1, sum(`number of buildings 2`), 0)) %>%
  mutate(blg_type = paste0(`building type`, " (N=", max(`number of buildings 3`), ")")) %>%
  ungroup()
...
res50$buffer_ts %>%
  group_by(treat, post) %>%
  summarize(area_acres = round(sum(as.vector(geo_area_meters)) / M2_PER_ACRE, 0)) %>%
  ungroup() %>%
  group_by(treat) %>%
  arrange(treat, post) %>%
